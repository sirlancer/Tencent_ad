...
# )
clf = lgb.LGBMClassifier(num_leaves=31, reg_alpha=0.0, reg_lambda=1, max_depth=-1, n_estimators=10000, objective='binary', subsample=0.7, colsample_bytree=0.7, subsample_freq=1, min_child_weight=50, random_state=2018, n_jobs=-1);
print("run grid search...")
searcher = GridSearchCV(estimator=clf, param_grid=param_grid, cv=3)
searcher.fit(train_x, train_y.values.reshape(-1,));
# clf.fit(train_x, train_y, eval_set=[(evals_x, evals_y)], eval_metric='auc',early_stopping_rounds=2000)
# res['score'] = clf.predict_proba(test_x)[:,1]
res['score'] = searcher.predict_proba(test_x)[:, 1]
res['score'] = res['score'].apply(lambda x: float('%.6f' % x))
res.to_csv('./submission3.csv', index=False)
os.system('zip ./baseline3.zip ./submission3.csv')
print("="*30)
print(searcher.best_params_)
joblib.dump(searcher, './GridSearchCV_model.m')
